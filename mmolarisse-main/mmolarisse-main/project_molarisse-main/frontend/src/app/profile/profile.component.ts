...
export class ProfileComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    let profileData: any = {
      ...
      genre: this.profileForm.get('genre')?.value
    };
      ...
      next: (updatedProfile) => {
        ...
        if (this.userRole.toUpperCase() === 'PATIENT') {
          const medicalData: FichePatient = {
            ...
            prenom: this.profileForm.get('prenom')?.value,
            age: this.profileForm.get('age')?.value,
            sexe: this.profileForm.get('genre')?.value,
            profession: this.profileForm.get('profession')?.value,
          ...
          this.patientService.updatePatientFiche(medicalData).subscribe({
            next: (updatedFiche) => {
              ...
              this.loading = false;
              this.snackBar.open('Profile and medical information updated successfully', 'Close', { duration: 3000 });
