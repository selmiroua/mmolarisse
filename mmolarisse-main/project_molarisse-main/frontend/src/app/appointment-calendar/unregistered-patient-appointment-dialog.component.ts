...
export class UnregisteredPatientAppointmentDialogComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    // Get the date value directly without formatting
    ...
    if (birthDate instanceof Date) {
      ...
      try {
        ...
        formattedBirthDate = birthDate; // Use as is
      }
    } else {
      console.error('Invalid birth date format:', birthDate);
      formattedBirthDate = new Date().toISOString().split('T')[0]; // Use current date as fallback
    }
